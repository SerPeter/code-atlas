...
The initial analysis suggested forking code-graph-rag as a starting point due to its Memgraph foundation and hierarchy model.

## Decision

We will build Code Atlas from scratch, borrowing design patterns and query patterns from existing tools rather than forking.

## Consequences

### Positive

- **Clean architecture**: Design the graph schema to support our full requirements from day one (docs, monorepo, external libs, pattern detection)

- **No legacy baggage**: Existing tools have architectural assumptions we'd need to fight (LLM-required queries, in-memory only, no hybrid search)
...
**Why rejected:**
- Graph model doesn't accommodate document nodes, external library stubs, monorepo hierarchy, or extended relationship types
- Architecture is built around LLM-translates-to-Cypher as the primary query path; our design inverts this (direct tools first, LLM optional)
