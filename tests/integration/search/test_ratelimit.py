...
from code_atlas.search.ratelimit import ConcurrencyGate, RateBudget, RateLimiter
...
@asynccontextmanager
async def _limiter(redis_settings):
    """One process's limiter: its own Valkey client, which the limiter only borrows."""
...
async def test_unlimited_never_blocks(settings, clean_keys):
    ...
    budget = RateBudget(model=clean_keys, rpm=0, tpm=0)
    async with _limiter(settings.redis) as lim:
        start = time.monotonic()
        for _ in range(20):
            await lim.acquire(budget, tokens=10_000)
...
async def test_drained_bucket_makes_the_caller_wait(settings, clean_keys):
    ...
    async with _limiter(settings.redis) as lim:
        ...
        start = time.monotonic()
        await lim.acquire(RateBudget(model=clean_keys, rpm=120, tpm=0))
...
async def test_budget_is_shared_between_limiters(settings, clean_keys):
    ...
    """
    budget_a, budget_b = RateBudget(model=clean_keys, rpm=10, tpm=0), RateBudget(model=clean_keys, rpm=10, tpm=0)
    async with _limiter(settings.redis) as a, _limiter(settings.redis) as b:
        # Capacity is 10; spend it all through `a`.
        for _ in range(10):
            await a.acquire(budget_a)
...
async def test_penalize_is_visible_to_another_limiter(settings, clean_keys):
    ...
    budget_a, budget_b = RateBudget(model=clean_keys, rpm=600, tpm=0), RateBudget(model=clean_keys, rpm=600, tpm=0)
    async with _limiter(settings.redis) as a, _limiter(settings.redis) as b:
        scale = await a.penalize(budget_a)
        assert scale == pytest.approx(0.5)

        await b.acquire(budget_b)  # b reads the shared factor on its next acquire
        assert budget_b.scale == pytest.approx(0.5)
...
async def test_penalty_cooldown_collapses_a_burst(settings, clean_keys):
    ...
    budget = RateBudget(model=clean_keys, rpm=600, tpm=0)
    async with _limiter(settings.redis) as lim:
        first = await lim.penalize(budget)
        second = await lim.penalize(budget)
        third = await lim.penalize(budget)
...
async def test_penalize_damps_the_concurrency_gate(settings, clean_keys):
    ...
    async with _limiter(settings.redis) as lim:
        assert gate.limit == 8
        await lim.penalize(RateBudget(model=clean_keys, rpm=0, tpm=0, gate=gate))
        assert gate.limit == 4
...
async def test_unreachable_valkey_degrades_instead_of_failing(settings, clean_keys):
    ...
    budget = RateBudget(model=clean_keys, rpm=10, tpm=10)
    async with _limiter(dead) as lim:
        start = time.monotonic()
        await lim.acquire(budget, tokens=5)  # must return, not raise, not hang
        assert time.monotonic() - start < 5.0
        assert await lim.penalize(budget) == pytest.approx(0.5), "local backoff should still apply"
...
async def test_token_budget_is_metered_separately(settings, clean_keys):
    ...
    async with _limiter(settings.redis) as lim:
        ...
        start = time.monotonic()
        await lim.acquire(RateBudget(model=clean_keys, rpm=0, tpm=6000), tokens=100)  # 6000/min = 100 tokens per second
