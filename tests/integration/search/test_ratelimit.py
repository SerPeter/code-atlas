...
async def test_unlimited_never_blocks(settings, clean_keys):
    ...
    lim = RateLimiter(settings.redis, model=clean_keys, rpm=0, tpm=0)
...
async def test_drained_bucket_makes_the_caller_wait(settings, clean_keys):
    ...
    lim = RateLimiter(settings.redis, model=clean_keys, rpm=120, tpm=0)
    r = await _raw(settings)
...
async def test_budget_is_shared_between_limiters(settings, clean_keys):
    ...
    """
    a = RateLimiter(settings.redis, model=clean_keys, rpm=10, tpm=0)
    b = RateLimiter(settings.redis, model=clean_keys, rpm=10, tpm=0)
...
async def test_penalize_is_visible_to_another_limiter(settings, clean_keys):
    ...
    a = RateLimiter(settings.redis, model=clean_keys, rpm=600, tpm=0)
    b = RateLimiter(settings.redis, model=clean_keys, rpm=600, tpm=0)
...
async def test_penalty_cooldown_collapses_a_burst(settings, clean_keys):
    ...
    lim = RateLimiter(settings.redis, model=clean_keys, rpm=600, tpm=0)
...
async def test_penalize_damps_the_concurrency_gate(settings, clean_keys):
    ...
    lim = RateLimiter(settings.redis, model=clean_keys, rpm=0, tpm=0, gate=gate)
    assert gate.limit == 8
    await lim.penalize()
    assert gate.limit == 4
    await lim.close()
...
async def test_unreachable_valkey_degrades_instead_of_failing(settings, clean_keys):
    ...
    lim = RateLimiter(dead, model=clean_keys, rpm=10, tpm=10)
    start = time.monotonic()
...
async def test_token_budget_is_metered_separately(settings, clean_keys):
    ...
    lim = RateLimiter(settings.redis, model=clean_keys, rpm=0, tpm=6000)
    r = await _raw(settings)
