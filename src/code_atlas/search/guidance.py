...
_RELATIONSHIP_SUMMARY: dict[str, str] = {
    ...
    RelType.CONTRADICTS: "Note <-> Note it disagrees with, unresolved (symmetric, from frontmatter)",
...
_USAGE_GUIDE: dict[str, str] = {
    ...
    "searching": (
        ...
        "Detail parameter: all search tools accept `detail='full'` to include source code, "
        "full docstrings, and caller/callee counts in results — avoids a separate get_context call."
    ),
    "cypher": (
        "# Cypher Query Guide\n"
        "\n"
        ...
        "**Frontmatter:** stored as one map property, not flattened keys — match nested keys directly, "
        "e.g. `MATCH (n:Note) WHERE n.frontmatter.metadata.type = 'feedback'` or "
        "`WHERE 'sre' IN n.frontmatter.audience`. Present on Note, DocFile and DocSection. Keys a Note "
        "consumes (id, kind, tags, derived_from, supersedes, contradicts, anchors) are NOT in the map — "
        "use n.kind / n.tags / the edges instead.\n"
        "**dbt:** models/snapshots are TypeDef `kind` in dbt_model|dbt_snapshot, uid "
        "`{project}:dbt.model.{name}`; sources are TypeDef `dbt_source` named `{source}.{table}`; "
        "macros are Callable `dbt_macro` at `{project}:dbt.macro.{name}`. `ref()`/`source()` become "
        "USES_TYPE, macro calls become CALLS, and a schema.yml `description:` is a DocSection "
        "linked by DOCUMENTS rather than a property on the model.\n"
