...
_USAGE_GUIDE: dict[str, str] = {
    ...
    "searching": (
        ...
        "Exclude toggles: `exclude_tests=false` to include test entities in results."
    ),
    "cypher": (
        "# Cypher Query Guide\n"
        "\n"
        "**UID format:** `{project_name}:{qualified_name}` (e.g., `myproj:pkg.mod.MyClass`).\n"
        "**Labels:** Project, Package, Module, TypeDef, Callable, Value, DocFile, DocSection, "
        "ExternalPackage, ExternalSymbol.\n"
